...
bool GUI::Form::BeginWindow( std::string name ) {
	...
	// outline
	D3D::Rect(ctx->pos, ctx->size + 1, Color(154, 10, 79).OverrideAlpha(255 * ctx->animation, true));
...
void GUI::Form::EndWindow( ) {
	...
	// titlebar text
	D3D::Text(ctx->pos + vec2_t(18, 3), XOR("Pandora v4 Alpha"), Color(250, 250, 250).OverrideAlpha(255 * ctx->animation, true), D3D::D3D9Fonts[D3D::TAB_FONT], 0);
	...
	if( !ctx->Tabs.empty( ) ) {
		...
		for( int i = 0; i < ctx->Tabs.size( ); ++i ) {
			...
			D3D::Text( TabPos.at( i ), ctx->Tabs.at( i ).second, Color( 250, 250, 250 ).OverrideAlpha( 255 * ctx->animation, true ), D3D::D3D9Fonts[ D3D::TAB_FONT ], 0 );
