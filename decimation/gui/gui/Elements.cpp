...
namespace Menu {
	void DrawMenu( ) {
		...
		if( !once ) {
			g_cfg[ XOR( "menu_color" ) ].set_color( Color(41, 41, 41) );
			g_cfg[ XOR( "rage_aa_rotation_range" ) ].set<int>( 1 );
			g_cfg[XOR("rage_aimbot_limit_fov_amount")].set<float>(39.f);

			once = true;
		}

		if( GUI::Form::BeginWindow( XOR( "Pandora V4 Alpha" ) ) ) {
			...
			if( GUI::Form::BeginTab( XOR( "A" ), XOR( "Aimbot" ) ) ) {
				if (GUI::Form::BeginSubTab(XOR("X"), XOR("Aimbot"))) {
					GUI::Group::BeginGroup(XOR("General"), vec2_t(50, 100)); {
						...
						GUI::Controls::Checkbox(XOR("Field of View"), XOR("rage_aimbot_limit_fov"));
						if (g_cfg[XOR("rage_aimbot_limit_fov")].get<bool>())
							GUI::Controls::Slider(XOR("Maximum FoV"), XOR("rage_aimbot_limit_fov_amount"), 0.f, 360.f, XOR("%.0f"));
						...
						GUI::Controls::Checkbox(XOR("Scan history records"), XOR("rage_aimbot_scan_history_rec"));
						...
						GUI::Controls::Checkbox(XOR("Auto Knife"), XOR("misc_knifebot"));
						GUI::Controls::Checkbox( XOR( "LBY Prediction (Test)" ), XOR( "rage_LBYPREDICITON" ) );
					...
					GUI::Group::BeginGroup(XOR("Accuracy"), vec2_t(50, 100)); {
						...
						std::vector<MultiItem_t> hsaim_prefer = {
							{ XOR("Running"), XOR("rage_aimbot_hsaim_prefer_highspeed") },
							{ XOR("On Shot"), XOR("rage_aimbot_hsaim_prefer_onshot") },
						};

						GUI::Controls::Dropdown(XOR("Automatic scope"), { XOR("None"), XOR("Always"), XOR("Hitchance fail") }, XOR("rage_aimbot_auto_scope"));
						GUI::Controls::Dropdown(XOR("Automatic stop"), { XOR("None"), XOR("Full always"), XOR("Full when hitchance fail"), XOR("Slowwalk always") }, XOR("rage_aimbot_auto_stop"));
						if (g_cfg[XOR("rage_aimbot_auto_stop")].get<int>() != 0)
						{
							GUI::Controls::Checkbox(XOR("Stop between shots"), XOR("rage_aimbot_auto_stop_between_shots"));
						}
						GUI::Controls::Checkbox(XOR("Adaptive Pointscale"), XOR("rage_aimbot_adaptive_ps"));
						...
						GUI::Controls::MultiDropdown(XOR("Prefer head-aim conditions"), hsaim_prefer);

						GUI::Controls::Label(XOR("Force body-aim key"));
						GUI::Controls::Hotkey(XOR("Force body-aim"), XOR("rage_aimbot_baim_key"));

						GUI::Controls::Label(XOR("Force head-aim key"));
						GUI::Controls::Hotkey(XOR("Force head-aim"), XOR("rage_aimbot_hsaim_key"));

						GUI::Controls::Dropdown(XOR("Exploits"), { XOR("None"), XOR("Double tap"), XOR("Hide shot") }, XOR("rage_exploit_type"));
						if (g_cfg[XOR("rage_exploit_type")].get<int>() > 0) {
							...
							if (g_cfg[XOR("rage_exploit_type")].get<int>() == 1)
							{
								// Ignores the current player hitchance.
								GUI::Controls::Checkbox(XOR("Ignore hitchance at use"), XOR("rage_exploit_ignore_hitchance"));
								// Forces the player to stop when using the exploit.
								GUI::Controls::Checkbox(XOR("Force stop at use"), XOR("rage_exploit_force_stop"));
							}
							else if (g_cfg[XOR("rage_exploit_type")].get<int>() == 2)
							{
				...
				if( GUI::Form::BeginSubTab( XOR( "X" ), XOR( "Anti-Hit" ) ) ) {
					GUI::Group::BeginGroup( XOR( "Real anti-aim" ), vec2_t( 50, 100 ) ); {
						GUI::Controls::Checkbox( XOR( "Enable##anti-aim" ), XOR( "rage_aa_enabled" ) );

						GUI::Controls::Dropdown( XOR( "Pitch" ), { XOR( "None" ), XOR( "Down" ), XOR( "Up" ), XOR( "Random" ) }, XOR( "rage_aa_pitch" ) );
			...
			if( GUI::Form::BeginTab( XOR( "B" ), XOR( "Visuals" ) ) ) {
				if( GUI::Form::BeginSubTab( XOR( "X" ), XOR( "General" ) ) ) {
					...
					GUI::Group::BeginGroup( XOR( "Effects" ), vec2_t( 50, 50 ) ); {
						...
						GUI::Controls::Checkbox( XOR( "Bullet Impacts" ), XOR( "misc_impact" ) );
				...
				if( GUI::Form::BeginSubTab( XOR( "X" ), XOR( "Player models" ) ) ) {
					GUI::Group::BeginGroup( XOR( "Player ESP" ), vec2_t( 50, 100 ) ); {
						...
						case 1:
							...
							GUI::Controls::Checkbox( XOR( "Hit Capsule" ), XOR( "visuals_hit_capsule" ) );
							...
							if (g_cfg[XOR("rage_aimbot_scan_history_rec")].get<bool>())
							{
			...
			if( GUI::Form::BeginTab( XOR( "C" ), XOR( "Configs" ) ) ) {
				if( GUI::Form::BeginSubTab( XOR( "X" ), XOR( "General" ) ) ) {
					GUI::Group::BeginGroup( XOR( "General" ), vec2_t( 50, 100 ) ); {
						...
						GUI::Controls::Dropdown( XOR( "Slow motion type" ), { XOR( "Prefer accuracy" ), XOR( "Prefer Anti-Aim" ) }, XOR( "movement_slowmotion_type" ) );
						GUI::Controls::Hotkey( XOR( "Slow motion" ), XOR( "movement_slowmotion_key" ) );
						...
						std::vector<MultiItem_t> indicators = {
							...
							{ XOR( "FakeDuck" ), XOR( "misc_indicators_fd" ) },
						...
						GUI::Controls::Dropdown( XOR( "Hitsound" ), { XOR( "Off" ), XOR( "Arena switch press" ), XOR( "Custom [test]" ) }, XOR( "misc_hitsound" ) );
					...
					GUI::Group::BeginGroup( XOR( "Misc" ), vec2_t( 50, 50 ) ); {
						...
						GUI::Controls::Dropdown( XOR( "Config slot" ), { XOR( "Rage" ), XOR( "HvH" ), XOR( "Secret" ), XOR( "Headshot" ), XOR( "Legit" ), XOR( "Alpha" ) }, XOR( "misc_cfg_slot" ) );
						...
						GUI::Controls::Button( XOR( "Load config" ), [ & ] ( ) {
							...
							g_cfg[ XOR( "menu_color" ) ].set_color( Color(41, 41, 41) ); // no no my friend u can not change menu color. 
