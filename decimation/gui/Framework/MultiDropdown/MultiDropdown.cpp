...
bool show_select_tabsss5 = false;
...
bool GUI::Controls::MultiDropdown( const std::string& name, std::vector< MultiItem_t > options ) {
	...
	Color ArrowClr(
		Color::Blend( Color::Palette_t::MenuColors_t::Outlines( ), g_cfg[ XOR( "menu_color" ) ].get_color( ).OverrideAlpha(70 * ctx->animation, true), ctx->MultiDropdownInfo._ArrowAnimation.at( GUI::Hash( name ) ) ) );
	...
	D3D::RectFilled( DrawPos, DrawSize, ArrowClr.OverrideAlpha( 100 * ctx->animation, true ) );
	...
	if (InputHelper::Hovered(ctx->pos, vec2_t(50, 50))) {
		...
		if (InputHelper::Pressed(VK_LBUTTON)) {
			show_select_tabsss5 = !show_select_tabsss5;
		}
