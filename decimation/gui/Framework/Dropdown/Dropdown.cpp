...
bool show_select_tabsss4 = false;
bool GUI::Controls::Dropdown( const std::string& name, std::vector< std::string > options, const std::string& var_name ) {
	...
	Color ArrowClr(
		Color::Blend( Color::Palette_t::MenuColors_t::Outlines( ), g_cfg[ XOR( "menu_color" ) ].get_color( ).OverrideAlpha(70 * ctx->animation, true), ctx->DropdownInfo.ArrowAnimation.at( GUI::Hash( name ) ) ) );
	...
	if (InputHelper::Hovered(ctx->pos, vec2_t(50, 50))) {
		...
		if (InputHelper::Pressed(VK_LBUTTON)) {
			show_select_tabsss4 = !show_select_tabsss4;
		}
