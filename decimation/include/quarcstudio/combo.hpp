...
namespace QGUI 
{
	...
	bool begincombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
	{
		...
		const float expected_w =210;
		const float w = (flags & ImGuiComboFlags_NoPreview) ? arrow_size : expected_w;

		const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w +0, label_size.y + style.FramePadding.y * 2.0f + 15));
		...
		const float value_x2 = ImMax(frame_bb.Min.x, frame_bb.Max.x - arrow_size);
			...
			window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 3, frame_bb.Min.y + 20), ImVec2(frame_bb.Max.x + 3, frame_bb.Max.y + 0), ImColor(1.000000f,0.705882f,0.000000f,1.000000f),6, 15); 
		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0/ 255.f , 0/ 255.f ,0/ 255.f ,255/ 255.f ));
		if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
			ImGui::RenderText(frame_bb.Min + style.FramePadding + ImVec2(6,17), preview_value);
		ImGui::PopStyleColor();

		ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(229/ 255.f , 229/ 255.f ,229/ 255.f ,255/ 255.f ));
		if (label_size.x > 0)
			ImGui::RenderText(ImVec2(frame_bb.Max.x+ style.ItemInnerSpacing.x +-210, frame_bb.Min.y + style.FramePadding.y +0), label);
		ImGui::PopStyleColor();
