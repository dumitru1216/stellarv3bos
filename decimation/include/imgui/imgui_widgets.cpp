...
bool ImGui::ButtonEx(const char* label, const ImVec2 & size_arg, ImGuiButtonFlags flags)
{
	...
	auto borderColor = ImColor(12, 12, 12, 255);
	auto topColor = ImColor(26, 26, 26, 255);
	auto bottomColor = ImColor(36, 36, 36, 255);
	auto topColorHovered = ImColor(36, 36, 36, 255);
...
bool ImGui::ColorBarEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	window->DrawList->AddRectFilledMultiColor(ImVec2(p.x + 2, p.y - 4), ImVec2(p.x + ImGui::GetWindowWidth() / 2, p.y - 2), black, black, black, black);
...
bool ImGui::MultiSelect(const char* label, bool _data[], std::vector<const char*> items, int fix)
{
	...
	{
		if (ImGui::BeginCombo(label, prevValue[fix].c_str(), 0, items.size()))
		{
			prevValue[fix] = "";
			std::vector<std::string> vec;
			for (auto i = 0; i < items.size(); ++i) {
				ImGui::Selectable(items.at(i), &_data[i], ImGuiSelectableFlags_DontClosePopups);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int items)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = window->Size.x - 64;
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(36, 0), window->DC.CursorPos + ImVec2(w, label_size.x > 0 ? label_size.y + 25 : 25));
    ...
    if (!popup_open)
        RenderArrow(cb_bb.Max - ImVec2(12, 12), ImGuiDir_Down, 0.5f);
    else
        RenderArrow(cb_bb.Max - ImVec2(12, 12), ImGuiDir_Up, 0.5f);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2 & size_arg)
{
	...
	// Render
	if (hovered)
		window->DrawList->AddRectFilled(bb.Min, bb.Max + ImVec2(-20, 1), ImColor(24, 24, 24, 255));
