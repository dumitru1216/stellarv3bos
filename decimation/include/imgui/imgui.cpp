    ...
    if (first_begin_of_the_frame)
    {
        ...
        } else
        {
            ...
            if (!(flags & ImGuiWindowFlags_NoBackground))
            {
                if (!(flags & ImGuiWindowFlags_ChildWindow) && !(flags & ImGuiWindowFlags_Popup) && !(flags & ImGuiWindowFlags_Tooltip)) {
                ...
                else if (flags & ImGuiWindowFlags_Popup || flags & ImGuiWindowFlags_Tooltip) {
                    // как я заебался эту хуйню делать сука.
                    ...
                    if (alpha != 1.0f)
                        bg_col = (bg_col & ~IM_COL32_A_MASK) | (IM_F32_TO_INT8_SAT(alpha) << IM_COL32_A_SHIFT);
                    window->DrawList->AddRectFilled(window->Pos + ImVec2(0, window->TitleBarHeight()), window->Pos + window->Size - ImVec2(20, 0), bg_col, window_rounding, (flags & ImGuiWindowFlags_NoTitleBar) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Bot);

                    window->DrawList->AddRect(window->Pos - ImVec2(0, 1), window->Pos + window->Size - ImVec2(20, 0), ImColor(12 / 255.f, 12 / 255.f, 12 / 255.f, g.Style.Alpha));
                }
            }
            ...
            if (flags & ImGuiWindowFlags_MenuBar)
            {
                ...
                window->DrawList->AddRectFilled(menu_bar_rect.Min + ImVec2(window_border_size, -1), menu_bar_rect.Max - ImVec2(window_border_size + 1, 0), GetColorU32(ImGuiCol_MenuBarBg), (flags & ImGuiWindowFlags_NoTitleBar) ? window_rounding : 0.0f, ImDrawCornerFlags_Top);
...
bool ImGui::BeginTabsBackground(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
	...
	if (first_begin_of_the_frame)
	{
		...
		else
		{
            ...
            // костыль чтоб блять дырок не было ебаных.
            window->DrawList->AddRectFilled(window->Pos + ImVec2(0, window->TitleBarHeight() - 3), window->Pos + window->Size - ImVec2(2, 530), backgroundColor, 0, false);

			window->DrawList->AddRect(window->Pos + ImVec2(0, window->TitleBarHeight() - 3), window->Pos + window->Size - ImVec2(0, 530), outerBorderColor, 0, false, 0.4);
			window->DrawList->AddRect(window->Pos + ImVec2(0, window->TitleBarHeight() - 3), window->Pos + window->Size - ImVec2(1, 530), innerBorderColor, 0, false, 0.4);
			window->DrawList->AddRect(window->Pos + ImVec2(0, window->TitleBarHeight() - 3), window->Pos + window->Size - ImVec2(2, 530), backgroundColor, 0, false, 0.4);
