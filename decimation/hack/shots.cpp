...
void Shots::OnHurt( IGameEvent* evt ) {
	...
	// hitmarker.
	if( g_loser.miscellaneous.hitmarker ) {
	...
	// hitsounds
	if( g_loser.miscellaneous.hitsound ) {
		int type = g_loser.miscellaneous.hitsound;
		...
		case 1:
			// arena switch press.
			g_csgo.m_sound->EmitAmbientSound( XOR( "buttons/arena_switch_press_02.wav" ), (g_loser.miscellaneous.hitsound_volume / 100.f ) );
			break;
		...
		case 2:
			...
			if( !g_cl.m_hitsounds.empty( ) ) {
				...
				std::string sound = std::string( XOR( "C:\\" ) ).append( g_cl.m_hitsounds[g_loser.miscellaneous.hitsound] );
				if( sound.size( ) > 2 ) {
					...
					// thank you again
					{
						...
						if( g_cl.m_update_sound ) {
							...
							g_hitsounds.AdjustHitsoundVolume( header, (g_loser.miscellaneous.hitsound_volume / 200.f ) );
	...
	// print this shit.
	if( g_loser.miscellaneous.log[0] ) {
		std::string out = tfm::format( XOR( "hit %s in the %s for %i [ %i remaining ]" ), name, m_groups[ group ], ( int )damage, hp );
		g_notify.add( out );
	}
