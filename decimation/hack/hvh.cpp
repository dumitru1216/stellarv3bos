...
void HVH::AntiAim() {
	...
	// load settings.
	m_pitch = g_loser.rage.rage_aa_pitch;
	m_yaw = g_loser.rage.rage_aa_yaw;
	m_yaw_fake = g_loser.rage.rage_aa_yaw_fake;
	m_jitter_range = g_loser.rage.rage_aa_jitter_range;
	m_rot_range = g_loser.rage.rage_aa_rotation_range;
	m_rot_speed = g_loser.rage.rage_aa_rotation_speed;
	m_rand_update = g_loser.rage.rage_aa_random_update;
...
void HVH::IdealPitch() {
	...
	g_cl.m_cmd->m_view_angles.x = state->m_flPitch;
}
...
void HVH::PrepareFake() {
	if (g_cl.m_ground) {
		if ((g_loser.rage.rage_aa_real_around_fake[0] && g_cl.m_speed < 3.5f) || (g_loser.rage.rage_aa_real_around_fake[1] && g_cl.m_speed > 3.5f && !g_movement.m_slow_motion) || (g_loser.rage.rage_aa_real_around_fake[3] && g_movement.m_slow_motion)) {
		...
		if (g_loser.rage.rage_aa_real_around_fake[2]) {
			m_use_real_around_fake = true;
		}
		else {
			m_use_real_around_fake = false;
	...
	// set lby angle in the choked commands.
		...
		if (!m_use_real_around_fake) {
			...
			if (g_loser.rage.rage_aa_real_around_fake_jitter) {
...
float HVH::ChooseReal() {
	...
	if (m_yaw > 0) {
		...
		if (g_loser.rage.rage_aa_jitter_type > 0) {
			// get the range from the menu.
			float range = m_jitter_range / 2.f;

			// offset.
			if (g_loser.rage.rage_aa_jitter_type == 1) {
...
void HVH::AdjustYaw() {
	...
	float custom_fake = std::clamp<float>(g_loser.rage.rage_aa_custom_fake, -60.f, 60.f);
...
void HVH::SendPacket() {
	// fake-lag enabled.
	if (g_loser.rage.rage_fakelag_enabled && !g_csgo.m_gamerules->m_bFreezePeriod() && !(g_cl.m_flags & FL_FROZEN)) {
		// limit of lag.
		int limit = std::clamp<int>((int)g_loser.rage.rage_fakelag_limit, 2, g_cl.m_max_lag);
		int mode = g_loser.rage.rage_fakelag_type;
		...
		if (delta < 0.1) {
			...
			if (!g_loser.rage.fakelag[0])
				limit = 2;
		}

		// move.
		if (g_loser.rage.fakelag[1] && delta > 0.1f && g_cl.m_speed > 0.1f && !test) {
			g_cl.m_should_lag = true;
		}

		// air.
		else if (g_loser.rage.fakelag[2] && !test && ((g_cl.m_buttons & IN_JUMP) || !(g_cl.m_flags & FL_ONGROUND))) {
			g_cl.m_should_lag = true;
		}

		// crouch.
		else if (g_loser.rage.fakelag[3] && g_cl.m_local->m_bDucking() && !test) {
			g_cl.m_should_lag = true;
		}
	...
	if (!g_hvh.test && g_cfg[XOR("cheat_mode")].get<int>() == 0) {
		if (g_cl.m_old_shot)
			*g_cl.m_packet = true;
	}
