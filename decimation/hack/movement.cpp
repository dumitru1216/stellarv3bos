...
void Movement::JumpRelated( ) {
	...
	if( !LastJumped && ShouldFake ) {
		...
		if( g_loser.miscellaneous.air_duck) {
...
void Movement::Strafe( ) {
	...
	if( g_cl.m_pressing_move && g_loser.miscellaneous.wasd_strafe) {
...
void Movement::AutoStop()
{
	...
	auto max_speed = 0.33f * (g_cl.m_local->m_bIsScoped() > 0 ? g_cl.m_weapon_info->flMaxPlayerSpeedAlt : g_cl.m_weapon_info->flMaxPlayerSpeed);

	if (g_cl.m_ground && g_loser.rage.rage_aimbot_auto_stop && !m_slow_motion) {
