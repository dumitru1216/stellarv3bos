...
void Visuals::ModulateWorld( ) {
	...
	if(g_loser.player_esp.world[0]) {
		const float world_darkness = 1.f - (g_loser.player_esp.world_night_darkness / 100.f );
		const float prop_darkness = 1.3f - (g_loser.player_esp.world_night_darkness / 100.f );
		const float prop_alpha = 1.f - (g_loser.player_esp.world_prop_opacity / 100.f );
	...
	// disable night.
		...
		const float prop_alpha = g_loser.player_esp.world_prop_opacity < 99.f ? g_loser.player_esp.world_prop_opacity * 0.01f : 1.f;
...
void Visuals::ThirdpersonThink( ) {
	...
	if( g_csgo.m_input->m_camera_in_third_person ) {
		...
		offset.z = g_loser.miscellaneous.thirdperson_distance;
...
void Visuals::HitmarkerScreen( ) {
	if ( g_loser.miscellaneous.hitmarker != 1 )
		return;
...
/*void Visuals::HitmarkerWorld( ) {
	if(g_loser.miscellaneous.hitmarker != 2 )
		return;
	...
	for( size_t i{ }; i < m_world_hitmarkers.size( ); ++i ) {
		...
		if( info.m_world_to_screen ) {
			...
			// hitmarker
			draw_lines( info.m_world_pos, Color::Palette_t::Red( ) );
		}
	}
}*/
...
void Visuals::think( ) {
	...
	if(g_loser.player_esp.remove_scope
		&& g_cl.m_local->alive( )
...
void Visuals::StatusIndicators( ) {
	...
	if(g_loser.rage.rage_fakelag_enabled && g_cfg[ XOR( "misc_indicators_lagcomp" ) ].get<bool>( ) ) {
	...
	if(g_loser.rage.rage_aimbot_enabled) {
	...
	for( size_t i{ }; i < indicators.size( ); ++i ) {
		...
		render::indicator.string(g_cl.m_width * 0.5f - TextSize.m_width * 0.5f, g_cl.m_height * 0.5f + 60 + (14 * i), indicator.color, indicator.text);
	}
}
...
void Visuals::PenetrationCrosshair( ) {
	...
	if( !g_loser.player_esp.penetration_crosshair || !g_cl.m_processing )
		return;
...
void Visuals::draw( Entity* ent ) {
	...
	else if( ( g_loser.player_esp.bomb[0] || g_cfg[ XOR( "visuals_misc_dropped_weapons" ) ].get<bool>( ) || g_cfg[ XOR( "visuals_misc_dropped_weapons_ammo" ) ].get<bool>( ) ) && ent->IsBaseCombatWeapon( ) && !ent->dormant( ) )
...
/*void Visuals::DrawProjectile(Weapon* ent) {
	...
	// draw decoy.
	// g_cfg[XOR("visuals_misc_projectiles")]
	if (ent->is(HASH("CDecoyProjectile")) (g_cfg[XOR("visuals_misc_projectiles"))
		render::pixel.string(screen.x, screen.y, col, XOR("DECOY"), render::ALIGN_CENTER);

	// draw molotov.
	else if (ent->is(HASH("CMolotovProjectile")) && g_Options.visuals_misc_projectiles)
		render::pixel.string(screen.x, screen.y, col, XOR("FIRE"), render::ALIGN_CENTER);

	else if (ent->is(HASH("CBaseCSGrenadeProjectile")) && g_Options.visuals_misc_projectiles) {
		...
		if (model) {
			...
			if (name.find(XOR("flashbang")) != std::string::npos && g_Options.visuals_misc_projectiles)
				render::pixel.string(screen.x, screen.y, col, XOR("FLASH"), render::ALIGN_CENTER);

			else if (name.find(XOR("fraggrenade")) != std::string::npos && g_Options.visuals_misc_projectiles) {
	...
	// find classes.
		...
		if (g_Options.esp_fire_enable)
			render::world_circle(origin, 170.f, 5.f, 1.f, g_Options.esp_fire_color);
		//render::sphere( origin, 120.f, g_Options.visuals_misc_projectiles_color );
		if (g_Options.visuals_misc_projectiles)
			render::pixel.string(screen.x, screen.y, col, XOR("FIRE"), render::ALIGN_CENTER);
	}

	else if (ent->is(HASH("CSmokeGrenadeProjectile"))) {
		if (g_Options.esp_smoke_enable && ent->m_vecVelocity().length_2d() == 0.f)
			render::world_circle(origin, 144.f, 5.f, 1.f, g_Options.esp_smoke_color);

		if (g_Options.esp_wireframe_smoke && ent->m_vecVelocity().length_2d() == 0.f)
			render::sphere(origin, 144.f, 5.f, 1.f, g_Options.esp_smoke_color);
		//render::sphere( origin, 144.f, g_Options.visuals_misc_projectiles_color );
		if (g_Options.visuals_misc_projectiles)
...
void Visuals::DrawProjectile(Weapon* ent) {
 ...
 Color col = g_loser.player_esp.projectiles_color;

	// draw decoy.

	if (ent->is(HASH("CDecoyProjectile")) && g_loser.player_esp.projectiles)
		render::pixel.string(screen.x, screen.y, col, XOR("DECOY"), render::ALIGN_CENTER);

	// draw molotov.
	else if (ent->is(HASH("CMolotovProjectile")) && g_loser.player_esp.projectiles)
		render::pixel.string(screen.x, screen.y, col, XOR("FIRE"), render::ALIGN_CENTER);

	else if (ent->is(HASH("CBaseCSGrenadeProjectile")) && g_loser.player_esp.projectiles) {
		const model_t* model = ent->GetModel();
		...
		if (model) {
			...
			if (name.find(XOR("flashbang")) != std::string::npos && g_loser.player_esp.projectiles)
				render::pixel.string(screen.x, screen.y, col, XOR("FLASH"), render::ALIGN_CENTER);

			else if (name.find(XOR("fraggrenade")) != std::string::npos && g_loser.player_esp.projectiles) {
				render::pixel.string(screen.x, screen.y, col, XOR("FRAG"), render::ALIGN_CENTER);
			}
		}
	...
	// find classes.
		...
		if (g_cfg[XOR("visuals_misc_projectiles_range_circlething")].get<bool>())
			render::world_circle(origin, 170.f, 5.f, 1.f, g_cfg[XOR("visuals_misc_projectiles_range_circlething_color")].get_color());
		//render::sphere( origin, 120.f, g_Options.visuals_misc_projectiles_color );
		if (g_loser.player_esp.projectiles)
		render::pixel.string( screen.x, screen.y, col, XOR( "FIRE" ), render::ALIGN_CENTER );
	}

	else if( ent->is( HASH( "CSmokeGrenadeProjectile" ) ) ) {
		if (g_cfg[XOR("visuals_misc_smoke_range_circlething")].get<bool>() && ent->m_vecVelocity().length_2d() == 0.f)
			render::world_circle(origin, 144.f, 5.f, 1.f, g_cfg[XOR("visuals_misc_smoke_range_circlething_color")].get_color());
...
void Visuals::DrawItem( Weapon* item ) {
	...
	if( g_loser.player_esp.bomb[0] && item->is( HASH( "CC4" ) ) )
...
void Visuals::DrawPlayer( Player* player ) {
	...
	if(g_loser.player_esp.enemies_radar && enemy && !dormant )
	...
	bool dormant_esp = enemy && g_loser.player_esp.enemies_dormant;
	...
	color = enemy ? g_loser.player_esp.enemies_box_color : g_loser.player_esp.team_box_color;
	...
	bool bone_esp = ( enemy && g_loser.player_esp.enemies_skeleton ) || ( !enemy &&  g_loser.player_esp.team_skeleton);
	if( bone_esp )
	...
	bool box_esp = ( enemy && g_loser.player_esp.enemies_box) || ( !enemy && g_loser.player_esp.team_box);

	// render box if specified.
	if( box_esp ) {

		const float calcHorizontal = 0.5f * ( g_loser.player_esp.box_horizontal * 0.01f );
		const float calcVertical = 0.5f * (g_loser.player_esp.box_vertical * 0.01f );
		const float offsetHorizontal = g_loser.player_esp.box_horizontal >= 99.f ? 1 : 0;
		const float offsetVertical = g_loser.player_esp.box_vertical >= 99.f ? 1 : 0;

		const bool outline = g_loser.player_esp.box_outline;
	...
	bool name_esp = ( enemy && g_loser.player_esp.name ) || ( !enemy && g_loser.player_esp.name_team);
	...
	if( name_esp ) {
		...
		Color clr = enemy ? g_loser.player_esp.name_color : g_loser.player_esp.name_team_color;
		// override alpha.
		clr.a( ) = low_alpha;
	...
	bool health_esp = ( enemy && g_loser.player_esp.enemies_health) || ( !enemy && g_loser.player_esp.team_health);
	...
	if( health_esp ) {
		...
		// render actual bar.
		render::rect( box.x - 5, y + h - fill, 2, fill, g_loser.player_esp.enemies_health_override ? g_loser.player_esp.enemies_health_color.OverrideAlpha( alpha, true )
			: Color( r, g, 0, alpha ) );
	...
	// draw flags.
	{
		...
		bool money = ( enemy && g_loser.player_esp.enemies_flags[0]) || ( !enemy && g_cfg[ XOR( "esp_team_flags_money" ) ].get<bool>( ) );
		bool armor = ( enemy && g_loser.player_esp.enemies_flags[1]) || ( !enemy && g_cfg[ XOR( "esp_team_flags_armor" ) ].get<bool>( ) );
		bool scope = ( enemy && g_loser.player_esp.enemies_flags[2]) || ( !enemy && g_cfg[ XOR( "esp_team_flags_scope" ) ].get<bool>( ) );
		...
		bool bomb = ( enemy && g_loser.player_esp.enemies_flags[3]) || ( !enemy && g_cfg[ XOR( "esp_team_flags_bomb" ) ].get<bool>( ) );
	...
	// draw weapon.
	if( ( enemy && g_loser.player_esp.enemies_weapon ) || ( !enemy && g_loser.player_esp.team_weapon) ) {
		...
		if( weapon ) {
			...
			if( data ) {
				...
				// ammo bar.
				if( max != -1 && ( enemy && g_loser.player_esp.enemies_ammo) || ( !enemy && g_cfg[ XOR( "esp_team_ammo" ) ].get<bool>( ) ) ) {
					...
					Color clr = enemy ? g_loser.player_esp.enemies_ammo_color : g_cfg[ XOR( "esp_team_ammo_color" ) ].get_color( );
					clr.a( ) = alpha;
...
void Visuals::DrawPlantedC4( Entity* ent ) {
	...
	mode_2d = g_loser.player_esp.bomb[1];
	mode_3d = g_loser.player_esp.bomb[2];
	if( !mode_2d && !mode_3d )
		return;
...
void Visuals::DrawSkeleton( Player* player, int opacity ) {
	...
	for( int i{ }; i < hdr->m_num_bones; ++i ) {
		...
		Color clr = player->enemy( g_cl.m_local ) ? g_loser.player_esp.enemies_skeleton_color : g_loser.player_esp.team_skeleton_color;
...
void Visuals::RenderGlow( ) {
	...
	for( int i{ }; i < g_csgo.m_glow->m_object_definitions.Count( ); ++i ) {
		...
		if( enemy && !g_loser.player_esp.enemy_glow)
			continue;

		if( local && !g_loser.player_esp.local_glow)
			continue;

		if( !enemy && !local && !g_loser.player_esp.team_glow)
			continue;

		// enemy color
		if( enemy ) {
			color = g_loser.player_esp.enemy_glow_color;
		}
		// local color
		else if( local ) {
			color = g_loser.player_esp.local_glow_color;
			opacity = 1.f;
		}
		// friendly color
		else {
			if( player != g_cl.m_local )
				color = g_loser.player_esp.team_glow_color;
		}
...
void Visuals::DrawBeams( ) {
	...
	if( !g_cl.m_processing ) {
		...
		for( size_t i{ impact_count }; i-- > 0; ) {
			...
			if( std::abs( curtime - game::TICKS_TO_TIME( impact->m_tickbase ) ) > g_loser.player_esp.bullet_beam_time ) {
			...
			if( is_final_impact ) {
				...
				beam_info.m_flLife = g_loser.player_esp.bullet_beam_time;
				...
				if( !impact->m_hit_player ) {
					beam_info.m_flRed = g_loser.player_esp.bullet_beam_color.r( );
					beam_info.m_flGreen = g_loser.player_esp.bullet_beam_color.g( );
					beam_info.m_flBlue = g_loser.player_esp.bullet_beam_color.b( );
				}

				else {
					beam_info.m_flRed = g_loser.player_esp.bullet_beam_hurt_color.r( );
					beam_info.m_flGreen = g_loser.player_esp.bullet_beam_hurt_color.g( );
					beam_info.m_flBlue = g_loser.player_esp.bullet_beam_hurt_color.b( );
				}

				// attempt to render the beam.
				if( !g_loser.player_esp.bullet_beam_type ) {
				...
				// attempt to render the line.
					...
					g_csgo.m_debug_overlay->AddLineOverlay( start, end, beam_info.m_flRed, beam_info.m_flGreen, beam_info.m_flBlue, false, g_loser.player_esp.bullet_beam_time);
				}
