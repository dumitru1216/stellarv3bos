...
void events::player_hurt( IGameEvent* evt ) {
    ...
    if( evt && g_loser.miscellaneous.log[1] ) {
		...
		if (attacker != g_cl.m_local && victim == g_cl.m_local) {
			...
			g_notify.add(tfm::format(XOR("harmed by %s in the %s for %i damage."), name, g_shots.m_groups[evt->m_keys->FindKey(HASH("hitgroup"))->GetInt()], evt->m_keys->FindKey(HASH("dmg_health"))->GetInt()));
		}
...
void events::item_purchase( IGameEvent* evt ) {
	...
	if( !g_loser.miscellaneous.log[2])
		return;
...
void events::player_given_c4( IGameEvent* evt ) {
	...
	if( !g_loser.miscellaneous.log[3])
		return;
...
void events::bomb_begindefuse( IGameEvent* evt ) {
	...
	if( !g_loser.miscellaneous.log[4])
		return;
