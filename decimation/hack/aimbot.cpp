...
void AimPlayer::OnNetUpdate(Player* player) {
	bool reset = (!g_loser.rage.rage_aimbot_enabled || player->m_lifeState() == LIFE_DEAD || !player->enemy(g_cl.m_local));
...
void AimPlayer::SetupHitboxes(LagComp::LagRecord_t* record, bool history) {
	...
	// prefer, lethal.
	if (g_loser.rage.aimbot_baim_prefer[0])
	...
	// prefer, in air.
	if (g_loser.rage.aimbot_baim_prefer[1] && !(record->m_fFlags & FL_ONGROUND))
		m_hitboxes.push_back({ HITBOX_BODY, HitscanMode::PREFER });
	...
	bool ignore_limbs = record->m_vecVelocity.length_2d() > 71.f && g_loser.rage.rage_aimbot_ignore_limbs;

	if (g_loser.rage.aimbot_hitbox[0]) {

		if (g_loser.rage.rage_aimbot_prefer_safepoint) {
			m_hitboxes.push_back({ HITBOX_HEAD, HitscanMode::PREFER_SAFEPOINT, true });
		}

		m_hitboxes.push_back({ HITBOX_HEAD, g_loser.rage.rage_aimbot_prefered_hitbox == 0 ? HitscanMode::PREFER : HitscanMode::NORMAL, false });
	}

	if (g_loser.rage.aimbot_hitbox[1]) {
		m_hitboxes.push_back({ HITBOX_NECK, HitscanMode::NORMAL, false });
	}

	if (g_loser.rage.aimbot_hitbox[2]) {
		if (g_loser.rage.rage_aimbot_prefer_safepoint) {
	...
	if (g_loser.rage.aimbot_hitbox[3]) {
		...
		m_hitboxes.push_back({ HITBOX_PELVIS, g_loser.rage.rage_aimbot_prefered_hitbox == 2 ? HitscanMode::PREFER : HitscanMode::NORMAL, false });
		m_hitboxes.push_back({ HITBOX_BODY, g_loser.rage.rage_aimbot_prefered_hitbox == 1 ? HitscanMode::PREFER : HitscanMode::NORMAL, false });
	}

	if (g_loser.rage.aimbot_hitbox[4] && !ignore_limbs) {
	...
	if (g_loser.rage.aimbot_hitbox[5]) {
	...
	if (g_loser.rage.aimbot_hitbox[6] && !ignore_limbs) {
...
void Aimbot::find() {
	...
	if (best.player && best.record && best.target) {
		...
		bool on = g_loser.rage.rage_aimbot_hitchance && g_cfg[XOR("cheat_mode")].get<int>() == 0;
		...
		if (can_scope) {
			// always.
			if (g_loser.rage.rage_aimbot_auto_scope == 1) {
				g_cl.m_cmd->m_buttons |= IN_ATTACK2;
				return;
			}

			// hitchance fail.
			else if (g_loser.rage.rage_aimbot_auto_scope == 2 && on && !hit) {
				g_cl.m_cmd->m_buttons |= IN_ATTACK2;
				return;
			}
		}
		...
		if (hit || !on) {
			...
			if (!g_tickbase.m_shift_data.m_should_attempt_shift || ((!g_cfg[XOR("rage_exploit_charged")].get<bool>() || g_cl.m_goal_shift == 13 || g_tickbase.m_shift_data.m_should_disable) && g_tickbase.m_shift_data.m_should_attempt_shift) || (g_cfg[XOR("rage_exploit_charged")].get<bool>() && g_cl.m_goal_shift == 7 && g_tickbase.m_shift_data.m_should_attempt_shift && !(g_tickbase.m_shift_data.m_prepare_recharge || g_tickbase.m_shift_data.m_did_shift_before && !g_tickbase.m_shift_data.m_should_be_ready))) {
				if (g_loser.rage.rage_aimbot_autofire) {
					// right click attack.
...
bool Aimbot::CheckHitchance(Player* player, const ang_t& angle, LagComp::LagRecord_t* record, int hitbox) {
	...
	auto cur_aim_angle = vec3_t(angle.x, angle.y, angle.z);

	if ((g_cl.m_shoot_pos - cur_aim_angle).length_2d() > g_cl.m_weapon_info->flRange)
		return false;

	// setup calculation parameters. 
	const auto id = g_cl.m_weapon->m_iItemDefinitionIndex();
	const auto round_acc = [](const float accuracy) { return roundf(accuracy * 1000.f) / 1000.f; };

	// store off inaccuracy / spread ( these functions are quite intensive and we only need them once ). 
	auto inaccuracy = g_cl.m_weapon->GetInaccuracy();
	auto spread = g_cl.m_weapon->GetSpread();
	...
	for (int i = 0; i < SEED_MAX; i++) {
		...
		vec3_t viewForward = g_cl.m_local->Weapon_ShootPosition() + (viewForward * g_cl.m_weapon_info->flRange);

		CGameTrace Trace;

		g_csgo.m_engine_trace->ClipRayToEntity(Ray(g_cl.m_local->Weapon_ShootPosition(), viewForward), MASK_SHOT | CONTENTS_GRATE, player, &Trace);
...
bool AimPlayer::SetupHitboxPoints(LagComp::LagRecord_t* record, BoneArray* bones, int index, std::vector< vec3_t >& points) {
	...
	// get hitbox scales.
	float scale = g_loser.rage.rage_aimbot_multipoint_scale * 0.01f;

	// big inair fix.
	if (!(record->m_fFlags) & FL_ONGROUND)
		scale = 0.7f;

	float bscale = g_loser.rage.rage_aimbot_stomach_scale * 0.01f;
	...
	if (bbox->m_radius <= 0.f) {
		...
		if (index == HITBOX_R_FOOT || index == HITBOX_L_FOOT) {
			...
			if (g_loser.rage.aimbot_multipoint[4]) {
	...
	// these hitboxes are capsules.
		...
		if (index == HITBOX_HEAD) {
			...
			if (g_loser.rage.aimbot_multipoint[0]) {
				...
				if (state && record->m_vecVelocity.length() <= 0.1f && record->m_angEyeAngles.x <= state->m_flPitch) {
		...
		// body has 5 points.
			...
			// back.
			if (g_loser.rage.aimbot_multipoint[2])
		...
		// other stomach/chest hitboxes have 2 points.
			...
			if (g_loser.rage.aimbot_multipoint[1])
				points.push_back({ center.x, bbox->m_maxs.y - r, center.z });
		}
			...
			// half bottom.
			if (g_loser.rage.aimbot_multipoint[3])
...
bool AimPlayer::GetBestAimPosition(vec3_t& aim, float& damage, int& hitbox, LagComp::LagRecord_t* record, float& tmp_min_damage) {
	...
	if (g_cl.m_weapon_id == ZEUS) {
		...
		dmg = g_loser.rage.rage_aimbot_minimal_damage;
		if (g_loser.rage.rage_aimbot_minimal_damage_hp)
		...
		pendmg = g_loser.rage.rage_aimbot_penetration_minimal_damage;
		if (g_loser.rage.rage_aimbot_penetration_minimal_damage_hp)
			pendmg = std::ceil((pendmg / 100.f) * hp);

		if (g_aimbot.m_override_damage && g_loser.rage.rage_aimbot_minimal_damage_override_pen) {
			pendmg = g_loser.rage.rage_aimbot_minimal_damage_override_pen;
		}

		pen = g_loser.rage.rage_aimbot_penetration;
	}
...
bool Aimbot::SelectTarget(LagComp::LagRecord_t* record, const vec3_t& aim, float damage) {
	...
	if (g_loser.rage.rage_aimbot_limit_fov) {
		...
		if (math::GetFOV(g_cl.m_view_angles, g_cl.m_shoot_pos, aim) > g_loser.rage.rage_aimbot_limit_fov_amount)
			return false;
	}

	switch (g_loser.rage.rage_aimbot_target_selection) {
...
void Aimbot::apply() {
	...
	if (m_target) {
		if (!g_loser.rage.rage_aimbot_silent_aim)
	...
	if (attack || attack2) {
		...
		if (m_target) {
			...
			if (m_record)
			{
				if (g_cl.m_goal_shift == 13 && g_tickbase.m_shift_data.m_should_attempt_shift && g_tickbase.m_shift_data.m_should_be_ready) {
					if (m_orig_tickbase == -1)
					{
						m_orig_tickbase = g_cl.m_local->m_nTickBase();
					}
					float correct = g_cl.m_latency + g_cl.m_lerp;
					static auto sv_maxunlag = g_csgo.m_cvar->FindVar(HASH("sv_maxunlag"));
					correct = std::clamp(correct, 0.f, sv_maxunlag->GetFloat());
					g_cl.m_local->m_nTickBase() = game::TIME_TO_TICKS(m_record->m_flSimulationTime + correct);
					if (m_history)
					{
		...
		// norecoil.
		if (g_loser.rage.rage_aimbot_norecoil)
