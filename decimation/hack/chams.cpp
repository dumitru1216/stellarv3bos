...
void Chams::init() {
	...
	glowarmsrace = g_csgo.m_material_system->FindMaterial("dev/glow_armsrace", nullptr);
	glowarmsrace->IncrementReferenceCount();

	glass = g_csgo.m_material_system->FindMaterial(XOR("models/inventory_items/cologne_prediction/cologne_prediction_glass"), nullptr);
	glass->IncrementReferenceCount();

	crystal = g_csgo.m_material_system->FindMaterial(XOR("models/inventory_items/trophy_majors/crystal_blue"), nullptr);
...
bool Chams::OverridePlayer(int index) {
	...
	if (enemy && g_loser.player_esp.chams_enemies)
		return true;

	// we have chams on friendly.
	else if (!enemy && g_loser.player_esp.chams_team)
		return true;
...
void Chams::RenderHistoryChams(int index) {
	...
	if (enemy) {
		...
		// get color.
		Color color = g_loser.player_esp.chams_enemy_backtrack_color;
...
void Chams::RenderPlayer(Player* player) {
	...
	if (player->m_bIsLocalPlayer()) {
		...
		else if (g_loser.player_esp.chams_local_enable) {

			Color color = g_loser.player_esp.chams_local_color;

			SetAlpha(color.a() / 255.f);

			// set material and color.
			//SetupMaterial( g_cfg[ XOR( "esp_chams_local_material" ) ].get<int>( ) == 0 ? debugambientcube : debugdrawflat, color, false );
			switch (g_loser.player_esp.chams_local_material) {
			case 0:
				break;
			case 1: SetupMaterial(g_chams.debugambientcube, color, false);
				break;
		...
		if (enemy && g_loser.player_esp.chams_enemy_backtrack) {
		...
		if (enemy && g_loser.player_esp.chams_enemies) {
			if (g_loser.player_esp.chams_enemies_invis) {
				...
				Color color = g_loser.player_esp.chams_enemies_invis_color;

				SetAlpha(color.a() / 255.f);
				switch (g_loser.player_esp.chams_enemies_invis_material) {
				case 0:
					break;
				case 1: SetupMaterial(g_chams.debugambientcube, color, true);
					break;
				case 2: SetupMaterial(g_chams.debugdrawflat, color, true);
					break;
				case 3: SetupMaterial(g_chams.glowarmsrace, color, true);
					break;
				case 4: SetupMaterial(g_chams.esoglow, color, true);
					break;
				case 5: SetupMaterial(g_chams.glass, color, true);
					break;
				case 6: SetupMaterial(g_chams.crystal, color, true);
					break;
				}
				c_material_var* tint = g_chams.esoglow->find_var(XOR("$envmaptint"));
				c_material_var* tint2 = g_chams.glowarmsrace->find_var(XOR("$envmaptint"));
				c_material_var* tint3 = g_chams.glowarmsrace->find_var(XOR("$envmaptint"));
				c_material_var* tint4 = g_chams.crystal->find_var(XOR("$envmaptint"));
			...
			Color color = g_loser.player_esp.chams_enemies_color;

			SetAlpha(color.a() / 255.f);
			switch (g_loser.player_esp.chams_enemies_material) {
			case 0:
				break;
			case 1: SetupMaterial(g_chams.debugambientcube, color, false);
				break;
			...
			if (g_loser.player_esp.chams_team_invis) {
				...
				Color color = g_loser.player_esp.chams_team_invis_color;

				SetAlpha(color.a() / 255.f);
				switch (g_loser.player_esp.chams_team_invis_material) {
				case 0:
					break;
				case 1: SetupMaterial(g_chams.debugambientcube, color, true);
					break;
			...
			Color color = g_loser.player_esp.chams_team_color;

			SetAlpha(color.a() / 255.f);
			switch (g_loser.player_esp.chams_team_material) {
			case 0:
				break;
			case 1: SetupMaterial(g_chams.debugambientcube, color, false);
				break;
