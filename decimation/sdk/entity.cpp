...
void Player::ModifyEyePosition( CCSGOPlayerAnimState* state, vec3_t* pos ) {
	...
	if( nHeadBone != -1 ) {
		vec3_t vecHeadPos(
			state->pBaseEntity->m_BoneCache( ).m_pCachedBones[ 8 ][ 0 ][ 3 ],
			state->pBaseEntity->m_BoneCache( ).m_pCachedBones[ 8 ][ 1 ][ 3 ],
			state->pBaseEntity->m_BoneCache( ).m_pCachedBones[ 8 ][ 2 ][ 3 ] );
