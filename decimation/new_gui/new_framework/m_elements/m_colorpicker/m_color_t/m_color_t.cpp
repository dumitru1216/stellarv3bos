...
/* setup colorpicker */
ColorPicker2::ColorPicker2( color_t* m_Select, float* m_Hue, bool m_Alpha ) {
	this->m_Select = m_Select;
	this->m_AlphaBar = m_Alpha;

	HSV NewColor = RGB_TO_HSV2( {
...
void ColorPicker2::Draw( )
{
	...
	if ( m_AlphaIn[ Gui::m_Control.GetIndex( ) ] > 0 )
	{
		...
		RGB hue_color_t = HSV_TO_RGB2( { ( double )*m_Hue * 360.f, 1.f, 1.f } );
