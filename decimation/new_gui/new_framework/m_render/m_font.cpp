...
HRESULT c_font::init_device_objects( IDirect3DDevice9* dev )
{
	...
	const uint32_t hr = this->dev->CreateTexture( this->text_width, this->text_height, 1,
		D3DUSAGE_DYNAMIC, D3DFMT_A4R4G4B4, D3DPOOL_DEFAULT, &this->texture, nullptr );
...
HRESULT c_font::draw_radial( float x, float y, uint32_t color1, uint32_t color2, const wchar_t* text, uint8_t flags )
{
	...
	while ( *text )
	{
		...
		if ( c != ' ' )
		{
			...
			const auto center = vec2_t( ( x - .5f ) + w / 2.f, ( y - .5f ) + h / 2.f );
			const auto center_vertex = ( vec2_t( ( x1 + x2 ) / 2.f, ( y1 + y2 ) / 2.f ) );
