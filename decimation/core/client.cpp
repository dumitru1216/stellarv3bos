...
ulong_t __stdcall Client::init(void* arg) {
	...
	// welcome the user.
	g_notify.add(tfm::format(XOR("Succesfully loaded! Have fun! %s")));
...
void Client::KillFeed( ) {
	...
	static bool old_setting = g_loser.miscellaneous.preserve_killfeed;
	...
	if( g_cl.m_local->alive( ) ) {
		...
		if( clear_death_notices_addr && death_notices_addr ) {
			...
			if( g_cl.m_round_flags == 1 ) {
				if( local_death_notice_time && g_loser.miscellaneous.preserve_killfeed) {
					*local_death_notice_time = std::numeric_limits<float>::max( );
				}
			}

			// if we disable the option, clear the death notices.
			if( old_setting != g_loser.miscellaneous.preserve_killfeed) {
				if( !g_loser.miscellaneous.preserve_killfeed) {
				...
				old_setting = g_loser.miscellaneous.preserve_killfeed;
			}
...
void Client::ClanTag( )
{
	...
	int iCurFrame = ( ( int )( g_inputpred.m_stored_variables.m_flCurtime * 1.f ) ) % ( szClanTag.size( ) * 2 );

	if (g_loser.miscellaneous.clantag ) {
...
void Client::StartMove( CUserCmd* cmd ) {
	...
	m_latency = g_csgo.m_engine->GetNetChannelInfo()->GetAvgLatency(INetChannel::FLOW_OUTGOING);
...
void update_keybinds() {
	...
	g_aimbot.m_override_damage = c_keyhandler::get().auto_check(g_loser.rage.rage_aimbot_minimal_damage_override_key, g_loser.rage.rage_aimbot_minimal_damage_override_key_style);
	g_aimbot.m_force_body = c_keyhandler::get().auto_check(g_loser.rage.rage_aimbot_baim_key, g_loser.rage.rage_aimbot_baim_key_style);
	g_tickbase.m_shift_data.m_should_attempt_shift = g_loser.rage.rage_exploit_type > 0 && c_keyhandler::get().auto_check(g_loser.rage.rage_exploit_key, g_loser.rage.rage_exploit_key_style);
}
...
void Client::UpdateAnimations( ) {
	...
	// save updated data.
	m_speed = state->m_flSpeedNormalized;
